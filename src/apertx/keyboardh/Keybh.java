...
public class Keybh extends InputMethodService{
 boolean shift;
 boolean caps;
 boolean lang;
 boolean sound;
 ...
 public View onCreateInputView(){
  ...
  kv.setOnKeyboardActionListener(new KeyboardView.OnKeyboardActionListener() {
   ...
   public void onKey(int p1,int[] p2){
    ...
    switch(p1){
     case Keyboard.KEYCODE_DELETE:
      ...
      if(sound) snd_del.play(1,1,1,1,0,1);
      break;
     case Keyboard.KEYCODE_SHIFT:
      ...
      if(lang) en_keyboard.setShifted(shift);
      else ru_keyboard.setShifted(shift);
      kv.invalidateAllKeys();
      break;
     case Keyboard.KEYCODE_DONE:
      ic.sendKeyEvent(new KeyEvent(KeyEvent.ACTION_DOWN,KeyEvent.KEYCODE_ENTER));
      if(sound) snd_ret.play(1,1,1,1,0,1);
      break;
     case -10:
     ...
     case -12:
      ...
      kv.invalidateAllKeys();
      if(lang) kv.setKeyboard(en_keyboard);
      else kv.setKeyboard(ru_keyboard);
      break;
     case -13:
     ...
     case 32:
      ...
      ic.commitText(" ",1);
      if(sound) snd_spc.play(1,1,1,1,0,1);
      break;
     default:
      char code=(char)p1;
      if(Character.isLetter(code)&&shift) code=Character.toUpperCase(code);
      if(!caps&&shift){
       shift=false;
       if(lang) en_keyboard.setShifted(shift);
       else ru_keyboard.setShifted(shift);
      ...
      if(sound) snd_def.play(1,1,1,1,0,1);
    }
   }
   ...
   public void onPress(int p1){
    if(vibrate) vibrator.vibrate(vibtime);
   }
 ...
 public void onWindowShown(){
  ...
  if(str.length()<=4) vibtime=Integer.parseInt(str);
  else vibtime=50;
 }
